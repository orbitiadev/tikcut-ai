...
export default function StudioProV2() {
  ...
  function changeEnd(raw: number) {
    if (!Number.isFinite(raw) || duration <= 0) return;
    clearOutput();
    setRangeEnd(clamp(raw, rangeStart + .1, duration));
  }
  ...
  async function autopilot() {
    ...
    await withBusy('autopilot', async () => {
      ...
      setZoomPlan(plan); setBrollQueries(queries); if (social) setMetadata(social); setTimeline(populateTimeline(createStudioTimeline(selectedDuration), plan, queries, local));
      setStatus('Autopilot renderizando: corte + silêncios + formato + Auto Zoom…');
      showOutput(await exportAutopilotMp4(file, rangeStart, rangeEnd, currentSilences, plan, setProgress, { removeSilence, vertical, autoZoom }), `tikcut-autopilot-${style}-${Date.now()}.mp4`, `Autopilot ${style}`);
    });
  }
  ...
  return <div className="studio-pro-page app-shell">
    ...
    <main className="studio-pro-grid">
      ...
      <section className="pro-card autopilot-card"><div className="section-title">7 · AUTOPILOT completo</div><div className="autopilot-options"><label><input aria-label="Transcrever se necessário" type="checkbox" checked={autoTranscribe} onChange={(e) => setAutoTranscribe(e.target.checked)}/>Transcrever se necessário</label><label><input aria-label="Remover silêncios" type="checkbox" checked={removeSilence} onChange={(e) => setRemoveSilence(e.target.checked)}/>Remover silêncios</label><label><input aria-label="9:16" type="checkbox" checked={vertical} onChange={(e) => setVertical(e.target.checked)}/>9:16</label><label><input aria-label="Auto Zoom" type="checkbox" checked={autoZoom} onChange={(e) => setAutoZoom(e.target.checked)}/>Auto Zoom</label></div><select aria-label="Estilo do Autopilot" value={style} onChange={(e) => setStyle(e.target.value as AutopilotStyle)}>{STYLES.map((item) => <option key={item} value={item}>{item}</option>)}</select><button className="button autopilot-run" disabled={!file || Boolean(busy)} onClick={() => void autopilot()}>{busy === 'autopilot' ? `Autopilot ${progress ? Math.round(progress * 100) : 0}%` : 'EXECUTAR AUTOPILOT'}</button><p className="helper">O plano continua editável na timeline; revise B-roll, texto e resultado antes de publicar.</p></section>
