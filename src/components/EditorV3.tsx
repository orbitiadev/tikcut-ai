  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div><div className="eyebrow">PRIVATE TIKTOK STUDIO</div><h1>TikCut <span>AI</span></h1></div>
      ...
      <main className="workspace">
        <aside className="sidebar card">
          ...
          <label className="dropzone"><input type="file" accept="video/*" onChange={(event) => void handleFile(event.target.files?.[0] ?? null)} /><strong>{file ? file.name : 'Clique aqui para escolher um vídeo'}</strong><span>MP4, MOV, WebM e formatos suportados pelo navegador. O arquivo fica no seu dispositivo.</span></label>
          ...
          {isLongSource && <p className="helper">Fonte de 1h+ detectada. Escolha o trecho e use 15s até 2:45, ou IN/OUT manual de até 10 minutos.</p>}
          ...
          <textarea value={transcript} onChange={(event) => setTranscript(event.target.value)} placeholder="Cole a transcrição do vídeo. O AutoCut usa o texto para sugerir trechos." rows={9} />
        ...
        <section className="stage card">
          <div className="stage-toolbar"><div><b>2 · Marcar e cortar</b><span>IN/OUT · prévia · corte real</span></div><div className="top-actions"><span className="pill muted">Original ou 9:16</span>{isLongSource && <span className="pill good">1h+ OK</span>}</div></div>
          <div className="phone-frame">
            {url ? <video ref={videoRef} src={url} controls playsInline onTimeUpdate={handleVideoTimeUpdate} onLoadedMetadata={(event) => {
              const d = Number.isFinite(event.currentTarget.duration) ? event.currentTarget.duration : 0;
              setDuration(d); setTrimStart(0); setTrimEnd(d > 0 ? Math.min(d, 60) : 0);
              setStatus(d >= 3600 ? `Fonte longa pronta: ${formatTime(d)}. Escolha 15s, 30s, 45s, 60s, 90s, 2:30 ou 2:45.` : `Vídeo pronto: ${formatTime(d)}. Ajuste IN/OUT e clique em “Cortar vídeo”.`);
        ...
        <aside className="inspector card">
          ...
          <p className="helper">O AutoCut usa a transcrição que você cola, ranqueia trechos e posiciona IN/OUT; depois você confirma em “Cortar vídeo”.</p>
          <div className="suggestions">{suggestions.length ? suggestions.map((item) => <article className="suggestion" key={item.id}><div className="score">{item.score}<small>/100</small></div><div className="suggestion-body"><b>{item.title}</b><p>{item.excerpt.slice(0, 150)}{item.excerpt.length > 150 ? '…' : ''}</p><div className="mini-metrics"><span>Hook {item.hook}</span><span>Clareza {item.clarity}</span><span>Emoção {item.emotion}</span></div><small>{item.reason}</small><button onClick={() => applySuggestion(item)}>Aplicar IN/OUT sugerido</button></div></article>) : <div className="empty-list">Cole uma transcrição e clique em “Analisar melhores cortes”.</div>}</div>
        </aside>
      </main>

      <footer><span>Corte, exportação e análise de silêncio são processados no dispositivo.</span><span>v0.5.0 · uso pessoal</span></footer>
    </div>
  );
}
