...
test('editor rejects non-video files without crashing', async ({ page }) => {
  ...
  await page.locator('input[type="file"]').setInputFiles({
    name: 'nao-e-video.txt',
    mimeType: 'text/plain',
    buffer: Buffer.from('arquivo invalido para o editor'),
  });
...
test('caption styles and transcript scoring remain interactive', async ({ page }) => {
  await page.goto('/');
  const text = 'Você precisa ver isto agora! Uma fruta encontra uma passagem secreta e pergunta o que existe do outro lado? No final, uma revelação muda completamente a história!';
  await page.locator('.sidebar textarea').fill(text);
...
test('Storyverse navigation can switch back to editor repeatedly', async ({ page }) => {
  await page.goto('/');
